...
public:
    ...
    HBShaper(const string& fontFile, FreeTypeLib* fontLib, int textSize) {
        lib = fontLib;
    ...
    std::vector<VMesh*> drawText(HBText& text, float x, float y, glm::u8vec4 const& color, glm::vec2& botright) {
        vector<VMesh*> meshes;
    ...
    void deleteMeshes(std::vector<VMesh*>& meshes) {
        while (!meshes.empty()) {
